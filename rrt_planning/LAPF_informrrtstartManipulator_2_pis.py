...
from module.improve_sample import JointAdaptiveSampler
...
class LAPF_InformedRRTStar:
    ...
    def step_forward_along_path(self):
        ...
        if not self.heuristic_path or self.heuristic_path_index >= len(self.heuristic_path) - 1:
            ...
            if np.random.random() < self.initial_path_bias:
                return self.goal
        # 前进到下一个路径点
        next_idx = min(self.heuristic_path_index + 2, len(self.heuristic_path) - 1)
    ...
    def optimize_phase_sample(self):
        if self.best_cost < float('inf'):
            return self.sample.adaptive_sample(self.best_cost,self.x_center,self.C)
                ...
                if best_parent is not None:
                    ...
                    if self.reach_goal(node_new):
                        ...
                        if not self.found_initial_path:
                            ...
                            self.sample = JointAdaptiveSampler(self.robot, self.start, self.goal,initial_path=path[0])
                            print(path[0])
                            self.best_cost = cost
                            self.best_path = path
    ...
    def path_pruning_3d(self, path_tuple):
        ...
        输入: (config_list, cartesian_path_list, euler_list)
        输出: (pruned_config, pruned_cartesian, pruned_euler)
        """
...
if __name__ == "__main__":
    ...
    from curve_generate import Bspline_2
    # 初始化PyBullet环境
    p.connect(p.GUI)
    env =Env()
    # 创建机器人
    ...
    # 定义起点和终点
    start_config = [0, 0.4*math.pi, 0.25*math.pi, 0, 0.35*math.pi, -1.57]
    ...
    def fix_robot_position(robot, start_config):
        for i, joint_id in enumerate(robot.doosan_joints):
            ...
            p.resetJointState(robot.robotID, joint_id, start_config[i])
