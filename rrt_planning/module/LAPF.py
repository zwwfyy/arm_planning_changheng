...
class HeuristicPathGenerator:
    def __init__(self, robot, x_start, x_goal, step_len=0.1, N_rays=2,
                 tau_free=8, eta=1.5):
        """
        robot: 机器人对象
        x_start: 起点关节坐标
    ...
    def generate(self, max_iters=1500, stuck_limit=3, wall_follow_steps=3,
    ...
    def check_collision(self, joint_config):
        ...
        try:
            ...
            for i in range(self.robot.DOF):
                p.resetJointState(self.robot.doosan_id, i, joint_config[i])

            # 执行碰撞检测
            p.performCollisionDetection()

            # 检查是否有接触点
            contact_points = p.getContactPoints(self.robot.doosan_id)
    ...
    def potential_field_direction(self, p_node, q_tar):
        ...
        for i in range(self.dim):
            dist_lower = p_node[i] - self.joint_lower[i]
            dist_upper = self.joint_upper[i] - p_node[i]
            if dist_lower < 0.5:
                rep[i] += 0.1 / (dist_lower + 0.1)
            if dist_upper < 0.5:
                rep[i] -= 0.1 / (dist_upper + 0.1)
